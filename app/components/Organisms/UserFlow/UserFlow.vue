...
const { list, loading = false } = defineProps<UserFlowProps>();
...
function runWorkout() {
  if (todayActivity.value) {
    const warmupList = activityStore.getWarmUpActivities(
      todayActivity.value.id ?? '',
    );
    const activityList = activityStore.getActivities(
      todayActivity.value.id ?? '',
    );

    const timerStore = useTimerStore();
    timerStore.setListActivities(
      warmupList && warmupList.length > 0 ? warmupList : activityList,
    );
...
const showAddActivityButton = computed(
  () => !loading && !userStore.isNormal && activityStore.missingDays.length > 0,
);
</script>
  ...
  <div v-else-if="list && list.length > 0">
    ...
    >
      <Card :highlight="isHighlightedCard(item)">
        <div>
          <p
            v-if="isHighlightedCard(item)"
            class="text-sm"
          >
            {{
              !userStore.isTrainer && isToday(item.dayOfWeek)
                ? 'Today session'
                : ''
            }}
          </p>
          ...
          <p
            class="text-sm md:text-md truncate whitespace-nowrap overflow-hidden"
          >
            {{ item.description }}
          </p>
        </div>
