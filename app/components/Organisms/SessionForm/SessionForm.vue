...
const { session = null } = defineProps<SessionFormProps>();

interface SessionFormEvents {
  save: [session: Session];
  delete: [session: Session];
}

defineEmits<SessionFormEvents>();

const activityStore = useActivityStore();

const isNew = computed(() => !session?.id);

function isDaySelected(selectedDay: number) {
  return session?.dayOfWeek === selectedDay;
}

async function saveSession() {
  if (activityStore.selectedSession && !activityStore.selectedSession.id) {
    const returnedSession = await activityStore.addSession(
      activityStore.selectedSession,
    );
    if (returnedSession) {
    ...
    <div class="w-full">
      <div>
        <div class="text-center">
          ...
          >
            Editing session:
            <strong>{{ getDayOfTheWeek(session?.dayOfWeek) }}</strong>
          </span>
        </div>
