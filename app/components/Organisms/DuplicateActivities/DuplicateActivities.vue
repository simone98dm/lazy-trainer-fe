...
interface DuplicateActivitiesProps {
  show: boolean;
  dayOfWeek?: number;
...
const props = defineProps<DuplicateActivitiesProps>();

const { show, existingForm, isTrainer, missingDays } = props;

defineEmits<{
  close: [];
  duplicate: [{ dayOfWeek: number; activities: Activity[] }];
}>();
    ...
    <template #content>
      <div class="flex flex-col">
        ...
        <div class="w-full flex flex-wrap justify-center gap-3">
          <span
            v-for="activity in existingForm"
            :key="activity.id"
