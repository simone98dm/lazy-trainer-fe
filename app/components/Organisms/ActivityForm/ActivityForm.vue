...
const { activity = null } = defineProps<ActivityFormProps>();
...
const nameError = computed(() =>
  !selectedActivity.value?.name ? 'Name is required' : '',
);

const descriptionError = computed(() =>
  selectedActivity.value?.description &&
  selectedActivity.value?.description.length > 100
    ? 'Description is too long'
    : '',
);
const isTimeBasedActivity = ref(
  Boolean(activity?.time) && Boolean(activity?.time !== 0),
);

const timeError = computed(() =>
  isTimeBasedActivity.value && !selectedActivity.value?.time
    ? 'Time is required'
    : '',
);

const repsError = computed(() =>
  !isTimeBasedActivity.value && !selectedActivity.value?.reps
    ? 'Reps are required'
    : '',
);
        ...
        <div class="flex flex-wrap -mx-3">
          <div class="w-full px-3 mb-6">
            <Input
              ...
              @change="
                (value: string) =>
                  activityStore.updateActivityValue('name', value)
              "
              id="activityName"
          ...
          <div class="w-full px-3 mb-6">
            <Input
              ...
              @change="
                (value: string) =>
                  activityStore.updateActivityValue('description', value)
              "
          ...
          <div class="w-full px-3 mb-6">
            ...
            >
              <div
                class="w-full sm:w-64 justify-between shadow rounded-full h-12 flex p-1 mb-3"
              >
                <button
              ...
              <div class="w-full sm:w-fit mb-3">
                <TimeSelector
                  ...
                  @time-selected="
                    (value: string) =>
                      activityStore.updateActivityValue('time', value)
                  "
                  :error="timeError"
                />
                <Input
                  ...
                  @change="
                    (value: string) =>
                      activityStore.updateActivityValue('reps', Number(value))
                  "
                  id="activityReps"
          ...
          <div class="w-full px-3 mb-6 flex items-center justify-center gap-4">
            <label
              class="tracking-wide"
              :for="`toggle-${activity?.id ?? 'new-activity'}`"
            >
              <span class="flex-left inline"> Is warm-up? </span>
            </label>
            <Switch
              class="flex-left inline"
              :id="`toggle-${activity?.id ?? 'new-activity'}`"
              :name="`toggleWarmup-${activity?.id ?? 'new-activity'}`"
              :checked="selectedActivity?.warmup"
              @toggle="
                (isWarmup: boolean) =>
                  activityStore.updateActivityValue('warmup', isWarmup)
              "
            />
          </div>
        </div>
